...
mr=filter(m, V10=='RP') %>% group_by(TEID) %>% summarize(te_bp=sum(V5-V4+1, na.rm=T), n_root_hs=n(), root_bp=sum(V14))
ms=filter(m, V10=='AP') %>% group_by(TEID) %>% summarize(te_bp=sum(V5-V4+1, na.rm=T), n_shoot_hs=n(), shoot_bp=sum(V14))
mrf=filter(flank, V10=='RP') %>% group_by(TEID) %>% summarize(flank_bp=sum(V5-V4+1, na.rm=T), flank_n_root_hs=n(), flank_root_bp=sum(V14))
msf=filter(flank, V10=='AP') %>% group_by(TEID) %>% summarize(flank_bp=sum(V5-V4+1, na.rm=T), flank_n_shoot_hs=n(), flank_shoot_bp=sum(V14))
